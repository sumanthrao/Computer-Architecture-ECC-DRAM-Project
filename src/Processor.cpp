    ...
    llc(llc), trace(trace_fname), memory(memory)
{
  ...
  int limit = 50000;
  bool inst_interval = false;
  if (int(get_insts())%limit == 0) {
      inst_interval = true;
  }
  if (no_core_caches) {
    more_reqs = trace.get_filtered_request(
        bubble_cnt, req_addr, req_type);
    req_addr = memory.page_allocator(req_addr, id, req_type != Request::Type::READ, inst_interval);
  } else {
